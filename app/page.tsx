...
/* ── DIAL CODES (sorted A-Z, Maroc default) ── */
const DIAL_CODES_RAW = [
...
const DIAL_CODES = [...DIAL_CODES_RAW].sort((a, b) => a.label.localeCompare(b.label, 'fr', { sensitivity: 'base' }));
const MAROC_IDX = DIAL_CODES.findIndex(d => d.label === 'Maroc');
...
function Inscription() {
  ...
  const [dialCode, setDialCode] = useState(DIAL_CODES[MAROC_IDX]?.code ?? '+212');
  const dialRef = useRef<HTMLDivElement>(null);
  const [dialOpen, setDialOpen] = useState(false);
  const [dialIdx, setDialIdx] = useState(MAROC_IDX);
  useEffect(() => {
  ...
  const reset = () => {
    ...
    setOffre('early');
    setDialCode(DIAL_CODES[MAROC_IDX]?.code ?? '+212');
    setDialIdx(MAROC_IDX);
  };
